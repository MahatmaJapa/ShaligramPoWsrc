...
#if defined(NDEBUG)
# error "Shaligram cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x73aa802ac74a6cebebab3d216ec0cfef73a42486b75df2b299f6b87fe41cb307");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Shaligram: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Shaligram Signed Message:\n";
...
{
    // Shaligram: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Shaligram: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Shaligram: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1516379428, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Showdown Vote in Senate on Friday With Government Shutdown at stake";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04667ec8aacd82c8bc2ebb91339f76d2045a99d85fb5e5cbc352e7ee6d6e2c70a6bd58cb74fa5578301f53d3980338a7a2585245259ba31968a34064cd3f13481e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516379428;
        ...
        assert(block.hashMerkleRoot == uint256("0x954707aeb37ff06b302d4ea7f84f35ac6443383bfb9a6a1fe1d783eddab8d3b0"));
...
unsigned char pchMessageStart[4] = { 0x15, 0x25, 0x67, 0xfe }; // Shaligram: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ShaligramMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ShaligramMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ShaligramMiner(CWallet *pwallet)
{
    printf("ShaligramMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("shaligram-miner");
    ...
    try { loop {
        ...
        printf("Running ShaligramMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ShaligramMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ShaligramMiner, pwallet));
}
